...
function handlePositionUpdate(position) {
    ...
    let isGeoRunActiveForLogic = false; // Egen variabel for å bestemme om vi skal bruke GeoRun-logikk for target
    ...
    // === GeoRun Spesifikk Målbestemmelse ===
    if (currentPostDataFromCore && currentPostDataFromCore.type === 'georun' &&
        ...
        if (!allGeoRunPoints || allGeoRunPoints.length === 0 || !runTargetIndices || runTargetIndices.length === 0) {
            ...
            isGeoRunActiveForLogic = false; // Kan ikke bestemme GeoRun-mål
        } else {
            isGeoRunActiveForLogic = true; // Vi er i en GeoRun-post, fortsett med GeoRun-logikk
            ...
            if (runState.awaitingGeoRunStartConfirmation) {
                ...
                if (currentTargetIndexInSequence < runTargetIndices.length) {
                 ...
                 // Første ankomst til posten, målet er startpunktet for GeoRun (for å trigge postReached)
                 ...
                 currentGeofenceRadius = GEOFENCE_RADIUS; // Standard radius for å "låse opp" posten
                 logToMobile(`GeoRun (handlePositionUpdate): Første ankomst. Mål: ${targetLocationDetails.name}, Radius: ${currentGeofenceRadius}`, "debug");
            } else { // Løp er ferdig, eller annen udefinert tilstand for GeoRun
                isGeoRunActiveForLogic = false;
            }
        }
    }

    // === Standard Målbestemmelse (hvis ikke GeoRun er i en spesiell tilstand) ===
    ...
    if (isCurrentTargetTheFinishLine) {
        ...
        if (runState.awaitingGeoRunStartConfirmation && targetLocationDetails.globalId === 'geoRunButtonEnable') {
            ...
            if (!currentTeamData.unlockedPosts[`post${currentGlobalIdOriginal}`]) {
                 ...
                 runState.awaitingGeoRunStartConfirmation = true; // Nå venter vi på knappetrykk
                 ...
                 canCurrentlyInteract = true; // Kan se instruksjoner og (deaktivert) knapp
            }
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayFinalResults() { /* ... (uendret, men DEBUG-logg oppdatert) ... */
        logToMobile(`DEBUG_V68: Displaying final results.`, "info");
    ...
    function resetPageUI(pageIdentifier, pageElementContext = null) { /* ... (uendret fra v54) ... */ }
    function resetAllPostUIs() { /* ... (uendret) ... */ }
    async function initializeTeam(teamCode, teamPassword) { /* ... (uendret fra v54, men DEBUG-logg oppdatert) ... */
        logToMobile(`DEBUG_V68: initializeTeam kalt med kode: ${teamCode}`, "info");
        // ... (resten av funksjonen uendret)
    ...
    function handleTeacherPassword(postNum, password) { /* ... (uendret fra v54) ... */ }
    function handleMinigolfSubmit(postNum) { /* ... (uendret fra v54) ... */ }
    function handlePyramidPointsSubmit(postNum, pointsStr) { /* ... (uendret fra v54) ... */ }
    function handleGeoRunLogic(isAtTargetPoint, targetPointId, currentGeoRunPostId = null) { /* ... (uendret fra v54) ... */ }
    function handleTaskCheck(postNum, userAnswer) { /* ... (uendret fra v54) ... */ }
    window.proceedToNextPostOrFinishGlobal = function() { /* ... (uendret fra v54) ... */ }
    function updateUIAfterLoad() { /* ... (uendret fra v54) ... */ }
    function handleFinishCodeInput(userAnswer) { /* ... (uendret fra v54) ... */ }

    // === EVENT LISTENERS (uendret fra v54) ===
