create table if not exists public.group_score_adjustments (
  id uuid primary key default gen_random_uuid(),
  rebus_id uuid not null references public.rebuses(id) on delete cascade,
  student_id uuid not null references public.students(id) on delete cascade,
  points integer not null check (points between -5 and 5 and points <> 0),
  reason text not null default '',
  created_by uuid references public.profiles(id),
  created_at timestamptz not null default now()
);

create index if not exists idx_group_score_adjustments_student_created
on public.group_score_adjustments(student_id, created_at desc);

create index if not exists idx_group_score_adjustments_rebus_created
on public.group_score_adjustments(rebus_id, created_at desc);

alter table public.group_score_adjustments enable row level security;

drop policy if exists "group_score_adjustments_select_members" on public.group_score_adjustments;
create policy "group_score_adjustments_select_members"
...
create policy "group_score_adjustments_manage_teachers"
